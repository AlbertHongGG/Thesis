...
- 每一張 diagram 都必須有且只有一個 entry edge：`[*] --> X : <event>`（其 target `X` 為該 diagram 的 Entry State / 路口）。
- 這個 entry edge **只用來宣告入口 state**（轉成 `Diagram.meta.entryStateId` + `State.type=start`），不得生成一般 transition（見「Entry transition handling」）。

### `%% verify:` (required)

- 若出現 `%% verify:` 行，必須附著在其上一條或多條 transition；若該段尚未出現 transition，則附著在最近一次出現的 state。
  - 這些驗證項目會寫入對應的 `Transition.validations`（跨圖跳轉時也同步寫入 `Connector.validations`），且必須使用結構化 assertion 物件格式。
- 若整份文件沒有 `%% verify:`，輸出時 `validations` 仍需保留為空陣列（不可省略欄位）。
...
例外（Base/Delta 互跳；required support）：
- 當跨圖跳轉是「Base Page ↔ Delta Page」的文件語意互跳時，允許 `<Target>` 使用目的 diagram 的識別（例如 `navigate ActivityListPage.Admin`）。
- 這種 `<Target>` 仍視為跨圖跳轉，必須被轉為 `invokes` connector（並保留 `navigateTarget` 原文以利追溯）。
...
### Entry transition handling (required)

Mermaid 的 entry edge（`[*] --> X : <event>`）只用來宣告「diagram 的入口 state」，不應被轉成一般 transition：

- 必須將 `X` 設為 `Diagram.meta.entryStateId`，並將 `X` state 的 `type` 設為 `start`。
- **不得** 為 entry edge 生成 `Transition` 物件（避免產生冗餘的自我轉移，例如 `X -> X` 的 `enterPage` / `enterFeature` / `enterApp`）。
...
```json
{
  ...
  "roles": ["member"],
  "narrative": {
    "summary": "Member 在活動詳情頁完成報名流程",
    "taskDescription": "觸發 register 事件並完成報名，成功後狀態需由 ViewingDetail 轉為 Registered。"
  },
  "validations": [
    {
      "id": "v.t.page_activity_member.0002.01",
      "type": "network-success",
      "description": "registration API returns 200 and count increments",
      "expected": "HTTP 200"
    }
  ],
  "intent": {
...
- `narrative.summary`: 必須精簡、可讀，聚焦本步核心目標。
- `narrative.taskDescription`: 必須具操作導向（要做什麼、做到什麼算完成），不可抽象空話。
- `validations`: 所有附著在此 transition 的 `%% verify:` 項目，需轉為 assertion 物件陣列（每個 verify 項目獨立成一個物件）。
...
`Transition.validations[]` 與 `Connector.validations[]` 共用以下格式：
...
- `id`: 條件 id，必須穩定可重現（建議 `v.<ownerId>.<seq>`）。
- `type`: 僅允許 `url-equals|url-includes|text-visible|text-not-visible|element-visible|element-not-visible|network-success|network-failed|semantic-check`。
- `description`: assertion 說明，應優先對應 transition/connector validations。
- `expected`: 預期值（可選，可為 null）。
...
```json
{
  ...
  "narrative": {
    "summary": "從入口依角色導向活動列表頁",
    "taskDescription": "在 Entry 的 Member 狀態觸發 enterActivities 導航，並成功進入 ActivityListPage.Loading。"
  },
  "validations": [
    {
      "id": "v.c.page_entry.invokes.page_activity_list.01",
      "type": "url-equals",
      "description": "navigate target should match /activities",
      "expected": "/activities"
    }
  ],
  "meta": {
...
```json
{
  ...
  "narrative": {
    "summary": "Entry 與活動列表頁存在階層關係",
    "taskDescription": "建立 parent/child 關聯，表示 page_activity_list 受 page_entry 統轄。"
  },
  "validations": [],
...
- `narrative.summary`: 必須精簡、可讀，聚焦本步核心目標。
- `narrative.taskDescription`: 必須具操作導向（要做什麼、做到什麼算完成），不可抽象空話。
- `validations`: 由對應跨圖 transition 的 `%% verify:` 項目轉換而來；`contains` 可為空陣列。
- `meta.reason`: 關聯原因描述。
...
這張圖只負責：
- 描述「進站」後的入口決策與第一跳（Entry 必須先決定 identity，再導向對應的第一個 Page）
- **不得** 在 Entry 內做任何「既有 session/token」的前提假設或自動分流（例如 `detectSessionMember`、`resolveRoleFromSession`）。
- Entry 導向「非 Guest 身分」必須透過 Step 1 Spec 定義的身分取得流程（Authentication Onboarding）所建立（例如 login/register/auth page/其他 spec 明確定義流程），因此 **不得** 硬性假設系統一定存在 `login`/`register` 或它們必然是獨立頁。
- 以跨圖 transition（`<Action> | navigate <Target>`）導向第一個 Page/Feature 的 **Entry State（路口）**。

Entry **不承擔各頁內部流程**；頁面內部狀態流轉由各 Page diagram 自行管理。

## State ownership & transition placement (required)
在解析/校驗時必須遵守以下硬性規則：
- 一個 state 只能屬於一張 diagram（禁止在某張 diagram 中混入另一張 diagram 的 state 作為「流程節點」）。
- 外部 state 只允許出現在 target（跨圖導向），且預設 target 必須是目的 diagram 的 Entry State（路口）；若要跨圖進入非 Entry State，必須在該 diagram 標題下方的說明段落中明確註明例外原因與 spec 追溯點（否則視為輸入格式錯誤）。
...
- `event` 為 `:` 後的標籤。
- `narrative.summary` 與 `narrative.taskDescription` 為必填，需由 event/from/to 語意生成可讀且可執行的敘述。
- `%% verify:` 的每個項目都必須轉成一筆 `validations[]` 物件（不可把多個 verify 合併成單一 description）。
...
- 目標 diagram 推導優先順序：
  ...
  2) 若無法由 `to` 推導（或 `to` 不符合命名），才退回用 `<Target>`：
    ...
    - `<Target>` 若為 route：仍可建立 connector（保留 `navigateTarget`），但若無法穩定推導目標 diagram，需回報為輸入不合格（因為本規範要求跨圖跳轉是 state→state，應可由 `to` 明確指出目的 state）。
...
此外，connector 本體必須包含：
- `narrative.summary` / `narrative.taskDescription`：由 action + 導航目標組成可讀、可執行的步驟敘述。
- `validations[]`：附著在此跨圖 transition 的 `%% verify:` 項目逐筆轉換後的 assertion 物件陣列。
