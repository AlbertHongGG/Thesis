...
const DOCUMENT_OVERVIEW_EMPTY_GLOBAL_CONTEXT = '（目前沒有額外背景脈絡）';
  ...
  return {
    ...
    buildPrompt(input) {
      return renderPromptTemplate(DOCUMENT_OVERVIEW_USER_PROMPT_TEMPLATE, {
        globalContextBlock: renderPromptBlock(clampContext(input.globalContext), DOCUMENT_OVERVIEW_EMPTY_GLOBAL_CONTEXT),
